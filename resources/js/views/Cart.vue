<template>
    <div class="container mx-auto py-10 px-4">
        ...
        <div v-else class="space-y-6">
            <div v-for="item in cart.items" :key="item.slug"
                ...
                <div class="flex-1">
                    ...
                    <p class="text-gray-700">
                        Price: ${{ item.price }}
                        <span v-if="item.old_price" class="line-through text-gray-400">${{ item.old_price }}</span>
                    </p>
                    ...
                    <div class="flex items-center gap-2 mt-3">
                        <button @click="cart.decreaseQuantity(item.slug)"
                            class="px-3 py-1 border rounded hover:bg-gray-100">
                            -
                        </button>
                        <input type="number" min="1" v-model.number="item.quantity"
                            class="w-16 text-center border rounded px-2 py-1" />
                        <button @click="cart.increaseQuantity(item.slug)"
                            class="px-3 py-1 border rounded hover:bg-gray-100">
                            +
                        </button>
                        <button @click="cart.removeFromCart(item.slug)"
            ...
            <div class="text-right mt-8">
                ...
                <div class="mt-4 space-x-2">
                    <button @click="cart.clearCart()" class="px-6 py-2 bg-red-500 text-white rounded hover:bg-red-600">
                        Clear Cart
                    </button>
                    <router-link to="/checkout" class="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700">
                        Checkout
                    </router-link>
                </div>
            </div>
